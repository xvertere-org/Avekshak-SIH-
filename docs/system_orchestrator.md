...
Phase 13 establishes the production integration and orchestration runtime for the **SIH26054 Aero Piston Engine Digital Twin & PHM System** (Rotax 914 UL/F reference architecture, MALE UAV application).
...
- **`LOADED_PRETRAINED`** + horizon $\in \{16, 32\}$: Usable forecast-assisted path for Phase 11 RUL handoff (`handoff_horizon_s = 16` or `32`).
- **`BLOCKED_UNAUTHENTICATED_GATED`**: When TimesFM HuggingFace weights are gated/unauthenticated in the environment, Phase 10 falls back to the deterministic Causal EWMA baseline. The status remains explicitly `BLOCKED_UNAUTHENTICATED_GATED`.
...
- **Forecast Mode Active**: **`BLOCKED_UNAUTHENTICATED_GATED`** (deterministic Causal EWMA baseline fallback; TimesFM gated weights unauthenticated in local environment)
...
The test suite contains 340 tests (316 baseline Phase 1–12 tests + 24 Phase 13 orchestrator integration tests). All 340 pass (100% green).
...
2. **Preserved Distinctions**:
   ...
   - **Pretrained TimesFM Weights**: TimesFM 3.0 foundation model weights are external and gated; because the local runtime environment does not have authenticated credentials, the orchestrator explicitly reports `BLOCKED_UNAUTHENTICATED_GATED` and uses the Phase 10 Causal EWMA baseline. The project makes no claim to having trained or fine-tuned TimesFM.

3. **Synthetic Simulation**: All telemetry is produced by the physics-informed mathematical engine simulator (reduced-order lumped-parameter grey-box prototype anchored to Rotax 914 UL/F reference specifications). It has not been validated against experimental test-cell or operational flight records.
