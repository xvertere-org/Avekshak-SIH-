...
def get_parameter_inventory() -> List[ParameterRecord]:
    ...
    return [
        ...
        # Phase 4B: Cooling Degradation (Tier C/D)
        ParameterRecord(
            name="k_cooling_max_loss",
            tier="Tier C/D",
            current_value=0.55,
            units="fraction",
            effect="Max cooling conductance loss fraction at severity=1.0; produces CHT elevation",
            calibration_status="Engineering assumption",
            reference_source="Phase 4B cooling degradation model calibration assumption",
        ),
        # Phase 4C: Lubrication Degradation (Tier C/D)
        ParameterRecord(
            name="k_lub_p_loss",
            tier="Tier C/D",
            current_value=0.55,
            units="fraction",
            effect="Max hydraulic oil pressure loss fraction at severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4C lubrication degradation model calibration assumption",
        ),
        ParameterRecord(
            name="k_lub_heat_gain",
            tier="Tier C/D",
            current_value=0.20,
            units="fraction",
            effect="Oil frictional heat generation gain fraction at severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4C lubrication degradation model calibration assumption",
        ),
        ParameterRecord(
            name="k_lub_cool_loss",
            tier="Tier C/D",
            current_value=0.20,
            units="fraction",
            effect="Oil cooler heat rejection reduction fraction at severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4C lubrication degradation model calibration assumption",
        ),
        # Phase 4D: Fuel / Injection Abnormality (Tier C/D)
        ParameterRecord(
            name="k_fuel_flow_lean",
            tier="Tier C/D",
            current_value=0.25,
            units="fraction",
            effect="Fuel mass flow reduction fraction at lean severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4D fuel injection abnormality model calibration assumption",
        ),
        ParameterRecord(
            name="k_fuel_flow_rich",
            tier="Tier C/D",
            current_value=0.30,
            units="fraction",
            effect="Fuel mass flow increase fraction at rich severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4D fuel injection abnormality model calibration assumption",
        ),
        ParameterRecord(
            name="k_comb_loss_lean",
            tier="Tier C/D",
            current_value=0.08,
            units="fraction",
            effect="Indicated combustion power reduction fraction at lean severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4D fuel injection abnormality model calibration assumption",
        ),
        ParameterRecord(
            name="k_comb_loss_rich",
            tier="Tier C/D",
            current_value=0.06,
            units="fraction",
            effect="Indicated combustion power reduction fraction at rich severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4D fuel injection abnormality model calibration assumption",
        ),
        ParameterRecord(
            name="k_egt_lean_gain_c",
            tier="Tier C/D",
            current_value=95.0,
            units="°C",
            effect="Steady-state EGT elevation at lean severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4D fuel injection abnormality model calibration assumption",
        ),
        ParameterRecord(
            name="k_egt_rich_drop_c",
            tier="Tier C/D",
            current_value=80.0,
            units="°C",
            effect="Steady-state EGT drop at rich severity=1.0",
            calibration_status="Engineering assumption",
            reference_source="Phase 4D fuel injection abnormality model calibration assumption",
        ),
    ]
